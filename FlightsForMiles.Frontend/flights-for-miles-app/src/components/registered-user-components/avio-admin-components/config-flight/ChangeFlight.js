...
import { changeFlight, loadFlights } from '../../../../redux/avio-admin/flight/flightAction'
...
function ChangeFlight(props) {
    ...
    const changeFlightForm = useFormWithFields({
            ...
            if (Validation()) {
                ...
                .then(response => {
                    if (response.status === 204) {
                        ...
                        props.setChangeIsOpen({ isOpen: !props.changeIsOpen.isOpen, currentFlight: props.changeIsOpen.currentFlight })
                ...
                .catch(error => {
                    console.error(error)
                    if (error.response.data.indexOf("(Updating unsuccessfully. Server not found flight for updating.)") !== -1) {
                        alert.show("Updating unsuccessfully. Server not found flight for updating.", {
                            type: 'error'
                        })
                    }
                    else {
        ...
        fields: [startTimeField, endTimeField, startHourField, endHourField, startLocationField, endLocationField,
            flightLengthField, flightTimeField, additionalInfoField, numOfTransfersField,
            allTransfersField, planeNameField, luggageWeightField]
    })

    function Validation() {
        if(startTimeField.value === "" && endTimeField.value === "" && startHourField.value === "" && endHourField.value === "" && 
            startLocationField.value === "" && endLocationField.value === "" && flightLengthField.value === "" && flightTimeField.value === "" && 
            additionalInfoField.value === "" && numOfTransfersField.value === "" && allTransfersField.value === "" && planeNameField.value === "" 
        ...
        if (startTimeField.value !== "" && endTimeField.value !== "" &&
            startHourField.value !== "" && endHourField.value !== "") {
    ...
    return (
        <Modal ariaHideApp={false} isOpen={props.changeIsOpen.isOpen} closeTimeoutMS={500}
            className="new-member-inner-reg" onRequestClose={() => props.setChangeIsOpen({
                        ...
                        <>
                            <div>
                                <span className="user-box">
                                    ...
                                    <label>Start date time ({props.changeIsOpen.currentFlight.startTime.split(' ')[0]})</label>
                                </span>
                                <span className="user-box">
                                    ...
                                    <label>End date time ({props.changeIsOpen.currentFlight.endTime.split(' ')[0]})</label>
                                </span>
                            </div>
                            <div>
                                <span className="user-box">
                                    <input type="time" value={startHourField.value} required={startHourField.isRequired}
                                        onChange={startHourField.handleChange} id="startHourField"
                                        placeholder={props.changeIsOpen.currentFlight.startTime} />
                                    <label>Start hour time ({props.changeIsOpen.currentFlight.startTime.split(' ')[1] + " " 
                                        + props.changeIsOpen.currentFlight.startTime.split(' ')[2]})</label>
                                </span>
                                <span className="user-box">
                                    <input type="time" value={endHourField.value} required={endHourField.isRequired}
                                        ...
                                        placeholder={props.changeIsOpen.currentFlight.endTime} />
                                    <label>End hour time ({props.changeIsOpen.currentFlight.endTime.split(' ')[1] + " " 
                                        + props.changeIsOpen.currentFlight.endTime.split(' ')[2]})</label>
                                </span>
                            </div>
                            <div>
                                <span className="user-box">
                            ...
                            <div>
                                ...
                                <span className="user-box">
                                    <input type="number" value={flightLengthField.value} required={flightLengthField.isRequired}
                                        ...
                                        placeholder={props.changeIsOpen.currentFlight.flightLengthKM} />
                            ...
                            <span className="user-box">
                                    <input type="number" value={numOfTransfersField.value} required={numOfTransfersField.isRequired}
                                        ...
                                        placeholder={props.changeIsOpen.currentFlight.numberOfTransfers} />
                            ...
                            <div>
                                <span className="user-box">
                                    ...
                                    <textarea type="text" value={additionalInfoField.value} required={additionalInfoField.isRequired}
                                        ...
                                        placeholder={props.changeIsOpen.currentFlight.additionalInformation} />
                                </span>
                            </div>
                            <div>
