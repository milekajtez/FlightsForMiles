...
function FinishOperations() {
  ...
  const bookingFlighForFriends = () => {
    if (friendship.friendsSelectedForBooking.length > 0) {
      if (
        ...
        dispatch(
          ...
          .then((response) => {
            if (response.status === 200) {
              alert.show(
                "Send requests for booking successfully. Your friends will get a message if your requests for booking are valid.",
                {
                  type: "success",
                }
              );
          ...
          .catch((error) => {
            ...
            if (
              ...
              error.response.data.indexOf("Server not found friend.") !== -1
            ) {
      ...
      alert.show("You didn't pick any friends", {
        type: "error",
      });
    }
  };
