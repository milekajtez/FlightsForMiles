  ...
  const changePassForm = useFormWithFields({
    onSubmit: (e) => {
      if (PasswordsVaidation()) {
        ...
        dispatch(
          ...
          .then((response) => {
            if (response.status === 204) {
              alert.show(
                "Update password successfully. Now you can login with new password.",
                {
                  type: "success",
                }
              );
        ...
        alert.show(
          "New password and confirm new password must have the same inputs.",
          {
            type: "error",
          }
        );
    ...
    <div className="profile-change reg-box">
      ...
      <form onSubmit={changePassForm.handleSubmit}>
        <div style={{ display: "inline-block" }}>
          <input
            type="password"
            value={newPassField.value}
            onChange={newPassField.handleChange}
            id="newPassField"
        ...
        <div style={{ display: "inline-block" }}>
          <input
            type="password"
            value={confirmNewPassField.value}
            onChange={confirmNewPassField.handleChange}
