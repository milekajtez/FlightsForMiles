...
function AirlineRegistration() {
          ...
          .catch((error) => {
            ...
            if (
              error.response.data.indexOf(
                "Please enter a different airline name"
              ) !== -1
            ) {
              alert.show(
                "Adding airline unsuccessfully. Please enter a different airline name",
                {
                  type: "error",
                }
              );
  ...
  function Validation() {
    if (
      isNaN(Number(houseNumberField.value)) ||
      parseInt(houseNumberField.value) <= 0
    ) {
      alert.show(
