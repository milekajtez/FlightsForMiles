...
import { cancelRequest, loadRequests } from '../../../../../redux/regular-user/friendship/friendshipAction'
...
function MyRequest(props) {
    ...
    const cancel = () => {
        ...
        .catch(error => {
            console.log(error)
            if(error.response.data.indexOf("(Canceling unsuccessfully. Request doesn't exsist.)") !== -1){
        ...
        <div>
            ...
            <div className="dropdown-item">
                ...
                <span className="text-right" style={{ float: 'right', marginTop: "2%" }}>
                    <input type="button" className="btn btn-danger btn-sm" value="Cancel request" onClick={() => cancel()}/>
                </span>
            </div>
        </div>
    )
