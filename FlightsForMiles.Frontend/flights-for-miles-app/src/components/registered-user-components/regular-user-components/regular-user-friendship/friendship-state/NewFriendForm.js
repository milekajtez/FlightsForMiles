...
function NewFriendForm(props) {
        ...
        .catch((error) => {
          ...
          if (
            error.response.data.indexOf(
              "(Sender of request doesn't exsist in database.)"
            ) !== -1
          ) {
            ...
            error.response.data.indexOf(
              "(Receiver of request doesn't exsist in database.)"
            ) !== -1
          ) {
            ...
            error.response.data.indexOf(
              "(Receiver of request is not regular user.)"
            ) !== -1
          ) {
            ...
            error.response.data.indexOf(
              "(You have this friendship request or you are already friend with user with entered username.)"
            ) !== -1
          ) {
            alert.show(
              "You have this friendship request or you are already friend with user with entered username.",
            ...
            error.response.data.indexOf(
              "(You can't send request yourself.)"
            ) !== -1
          ) {
  ...
  return (
    <Modal
      ...
      onRequestClose={() => props.newFriendIsOpen(false)}
      style={{
        overlay: {
          position: "fixed",
          top: 0,
