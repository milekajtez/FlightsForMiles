      ...
      .catch((error) => {
        ...
        if (
          error.response.data.indexOf(
            "(Deleting unsuccessfully. Selected flight doesn't have any ticket or it dosn't exsist or ticket is already purchased.)"
          ) !== -1
        ) {
          alert.show(
            "Deleting unsuccessfully. Selected flight doesn't have any ticket or it dosn't exsist or ticket is already purchased.",
              ...
              <tr key={flight.flightID}>
                ...
                <td>
                  <button
                    ...
                    onClick={() => showTickets(flight.flightID)}
                  >
                    <i className="far fa-eye"></i> VIEW TICKETS
                  </button>
                  &nbsp;
                  <button
                    className="btn btn-secondary"
                    onClick={() =>
                      setAddTicket({ isOpen: true, flightID: flight.flightID })
                    }
                  >
                    <i className="fas fa-plus"></i> ADD NEW TICKET
                  </button>
                  &nbsp;
                  <button
                    className="btn btn-danger"
                    onClick={() => deleteAllCurrentTickets(flight.flightID)}
                  >
