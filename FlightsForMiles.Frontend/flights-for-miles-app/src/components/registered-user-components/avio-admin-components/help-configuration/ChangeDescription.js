...
function ChangeDescription(props) {
  ...
  const changeDescriptionForm = useFormWithFields({
    onSubmit: (e) => {
      if (Validation()) {
          ...
          .catch((error) => {
            ...
            if (
              error.response.data.indexOf(
                "(Update unsuccessfully. Server not found any pplication description.)"
              ) !== -1
            ) {
              alert.show(
                "Update unsuccessfully. Server not found any pplication description.",
        ...
        alert.show(
          "You must enter some information if you want to change application description.",
          {
            type: "info",
          }
        );
  ...
  return (
    <Modal
      ariaHideApp={false}
      isOpen={props.changeDescriptionIsOpen}
      closeTimeoutMS={500}
      className="new-member-inner-login"
      onRequestClose={() => props.setChangeDescriptionIsOpen(false)}
      style={{
        overlay: {
          position: "fixed",
          top: 0,
